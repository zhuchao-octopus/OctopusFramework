...
public class DLNAUtil {
    ...
    public static void selectDLNADevice(Device device)
    {
        DLNAContainer.getInstance().setSelectedDevice(device);
    }

    public static Device getDLNADevice(Device device)
    {
        return DLNAContainer.getInstance().getSelectedDevice();
    }

    public static void shareTo(Device device,String url)
    {
        DLNAController.play(device,url);
    }
}
