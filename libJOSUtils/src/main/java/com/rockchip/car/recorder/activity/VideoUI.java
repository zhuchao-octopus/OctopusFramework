...
public class VideoUI extends AVideoUI {
	...
	public void scaleSurface(int id) {
		android.widget.FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) mSurfaceViews[id]
				.getLayoutParams();
	...
	@Override
	public void setPreviewFormat(int id, int format) {
		if (mSurfaceViews != null && mSurfaceViews.length > id
				&& mSurfaceViews[id] != null) {
	...
	public void setRenderSize(int id, int width, int height) {
		if (mCameraService == null
				|| mCameraService.getCameraToSurface() == null)
			return;
		int surfaceId = (int) mCameraService.getCameraToSurface().get(id);
		SLog.d(TAG, "VideoUI::setRenderSize. id:" + id + "; surfaceId:"
				+ surfaceId + "; width:" + width + "; height:" + height);
		if (mSurfaceViews != null && mSurfaceViews.length > surfaceId
				&& mSurfaceViews[surfaceId] != null) {
	...
	public void requestRender(int id, byte[] data) {
		...
		if (mSurfaceViews != null && mSurfaceViews.length > surfaceId
			...
			if (!result) {
				SLog.d(TAG, "VideoUI::requestRender. id:" + id + "data:"
						+ (data != null ? data.length : null));
				mCameraService.addCallbackBuffer(id,
						BufferManager.getInstance(id).pull());
			}
		}
	}
	...
	public void previewSwitch() {
		if (mCameraService == null
				|| mCameraService.getSurfaceToCamera() == null)
			return;
	...
	AVideoUI.ICallbackRender mCallbackRender = new AVideoUI.ICallbackRender() {
		...
		public void addCallBackBuffer(int id, byte[] data) {
			mCameraService.addCallbackBuffer((int) mCameraService
					.getSurfaceToCamera().get(id), data);
		}
	};
}
