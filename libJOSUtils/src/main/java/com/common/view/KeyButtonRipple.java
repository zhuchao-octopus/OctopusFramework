...
public class KeyButtonRipple extends Drawable {
    ...
    public static final Interpolator ALPHA_OUT = new PathInterpolator(0f, 0f,
            0.8f, 1f);

    private final Interpolator mInterpolator = new LogInterpolator();
    private final Interpolator mAlphaExitInterpolator = ALPHA_OUT;
    private boolean mSupportHardware;
    private final View mTargetView;

    private final HashSet<Animator> mRunningAnimations = new HashSet<Animator>();
    private final ArrayList<Animator> mTmpArray = new ArrayList<Animator>();

    public KeyButtonRipple(Context ctx, View targetView) {
        mMaxWidth = 126;// ctx.getResources().getDimensionPixelSize(R.dimen.key_button_ripple_max_width);
    ...
    private void drawSoftware(Canvas canvas) {
        if (mGlowAlpha > 0f) {
            ...
            canvas.drawRoundRect(cx - rx, cy - ry, cx + rx, cy + ry, corner,
                    corner, p);
        }
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (mFocused) {
            ...
            canvas.drawRect(this.getBounds(), mFocusPaint);
        } else {
            if (mSupportHardware) {
...
//	}
    ...
    private void enterSoftware() {
        ...
        ObjectAnimator scaleAnimator = ObjectAnimator.ofFloat(this,
                "glowScale", 0f, GLOW_MAX_SCALE_FACTOR);
    ...
    /**
     * Sets the left/top property for the round rect to {@code prop} depending
     * on whether we are horizontal or vertical mode.
     */
    ...
    /**
     * Sets the right/bottom property for the round rect to {@code prop}
     * depending on whether we are horizontal or vertical mode.
     */
    ...
    private void enterHardware() {
        ...
        final RenderNodeAnimator startAnim = new RenderNodeAnimator(
                getExtendStart(), getExtendSize() / 2 - GLOW_MAX_SCALE_FACTOR
                * getRippleSize() / 2);
        ...
        final RenderNodeAnimator endAnim = new RenderNodeAnimator(
                getExtendEnd(), getExtendSize() / 2 + GLOW_MAX_SCALE_FACTOR
                * getRippleSize() / 2);
    ...
    private void exitHardware() {
        ...
        final RenderNodeAnimator opacityAnim = new RenderNodeAnimator(
                mPaintProp, RenderNodeAnimator.PAINT_ALPHA, 0);
