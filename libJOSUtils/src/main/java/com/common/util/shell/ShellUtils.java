...
public class ShellUtils {
    ...
    //	private final static boolean USE_SEANOVO_CMD_MODE = true;
    ...
    public static List<String> seanovoCmds = new ArrayList<String>();
    public static Map<String, SeanovoCmdCallback> seanovoCmdCallbacks = new HashMap<String, SeanovoCmdCallback>();

    public static void execSeanovoCmd(String cmd, SeanovoCmdCallback callback) {
        seanovoCmds.add(cmd);
        seanovoCmdCallbacks.put(cmd, callback);
        if (!isRunning && seanovoCmds.size() > 0) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    String key = "";
                    while (true) {
                        if (seanovoCmds.size() < 1) {
                            isRunning = false;
                            break;
                        }
                        command = seanovoCmds.remove(0);
                        key = command;
                        if (command == null) {
                            continue;
                        }
                        command = "start akd:" + command.replace(" ", ":");
                        int result = -1;
                        try {
                        ...
                        SeanovoCmdCallback callback = seanovoCmdCallbacks.remove(key);
                        if (callback != null) {
                            callback.notifyExecFinish();
                        }
                        if (seanovoCmds.size() > 0) {
                            continue;
                        } else {
                            isRunning = false;
                            break;
    ...
    public static boolean getAkdRunStatus() {
        CommandResult result = ShellUtils.execCommand("getprop | grep -w akd", false);
        if (result != null && result.toString().contains("running")) {
            return true;
        }
        return false;
    }
