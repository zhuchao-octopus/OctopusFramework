...
public class Kernel {
    ...
    // no need system id, but ak system only

    public static void doKeyEvent(int value) {
        Util.setFileValue(KEY, value);
    }

    public static void doKeyEvent(int value, int down) {
        Util.setFileValue(KEY_EX, new byte[]{(byte) value, (byte) down});
    }

    public static void doKeyEventEx(int value, int time) {
        Util.setFileValue(KEY_EX, new byte[]{(byte) value, (byte) 1});
        Util.doSleep(time);
        Util.setFileValue(KEY_EX, new byte[]{(byte) value, (byte) 0});
    }
