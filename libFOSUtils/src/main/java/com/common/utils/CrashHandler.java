...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
         ...
         * 如果用户没有处理则让系统默认的异常处理器来处理 mDefaultHandler.uncaughtException(thread,
         * ex); } else { SystemClock.sleep(3000); // 退出程序
    ...
    private boolean handleException(Throwable ex) {
        Log.e(TAG, mContext.getPackageName() + ":" + ex);
    ...
    // 10) + "-" +
    // time.substring(11, 13) + time.substring(14, 16) + time.substring(17);
    // String fileName = mContext.getPackageName() + ".crash_" + time + ".log";
    // if (com.common.util.Util.udiskIsMounted()) {
    ...
    // return "/udisk" + File.separator + "y101log" + File.separator;
    // }
    //
    // /**
    // * 文件删除
